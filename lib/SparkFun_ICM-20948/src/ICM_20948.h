...
// These lines let the code compile cleanly on as many SAMD boards as possible.
  ...
  const __FlashStringHelper *statusString(ICM_20948_Status_e stat = ICM_20948_Stat_NUM); // Returns a human-readable status message. Defaults to status member, but prints string for supplied status if supplied
